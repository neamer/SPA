...
public:
	...
	friend std::ostream& operator<<(std::ostream& stream, Queue<Type>& list)
	{
		list.Print(stream);
		return stream;
	}
...
template <class Type>
class SeqQueue : public Queue<Type>
{
	Type* m_data;
	int m_counter;
	int m_max_size;
...
public:
	...
	friend std::ostream& operator<<(std::ostream& stream, SeqQueue<Type>& list)
	{
		list.Print(stream);
		return stream;
	}
...
public:
	...
	virtual void Print(std::ostream& stream) override
	{
		stream << "Linked Queue object at " << this << std::endl;
		stream << "==================================START\n";
	...
	friend std::ostream& operator<<(std::ostream& stream, LinkedQueue<Type>& list)
	{
		list.Print(stream);
		return stream;
	}
...
void TestQueue()
{
	Queue<int>* queue = new SeqQueue<int>;
	Queue<int>* queue2 = new LinkedQueue<int>;
